...
// --- Sample Data (Need consistent data for all order table components) ---
// Note: Assuming a few orders have 'Processing' status to demonstrate the filter.
const initialOrdersData = [
  ...
  { id: 4, customer: 'Dashrath Yadav', email: 'dashrathsing@gmail.com', phone: '7415096947', address: 'Ganverat Birad, Dinlorigas, PIN: 588525', items: 'woman dress\nBrand Tanishq | SKL/U | HSN 001 M4', amount: '3530', date: '12/11/2025', status: 'Processing' }, // <-- Processing
  { id: 5, customer: 'Tanish', email: 'tanishc@gmail.com', phone: '9000444139', address: 'sindagi\nsindagi\nPIN: 588128', items: 'Letter Head (Min Order qty 10 books)', amount: '700', date: '9/11/2025', status: 'Processing' }, // <-- Processing
...
export default function ProcessingOrdersTable() {
  ...
  // 1. Filter by Status: Only include 'Processing' orders
  const processingOrders = initialOrdersData.filter(order => order.status === 'Processing');

  // 2. Apply search term filter to the processing orders
  ...
  const OrderRow = ({ order }) => {
    ...
    const avatarColor = ['#800080', '#dc3545', '#3d85c6'][order.id % 3]; // Color based on ID
  ...
  return (
    <div className="flex-1 p-6 lg:p-8 bg-gray-100 min-h-screen">
      <div className="orders-container bg-white rounded-lg shadow-xl p-6">
        ...
        {/* Orders Header: Changed to Processing Orders */}
        <div className="flex justify-between items-center pb-4 border-b border-gray-100">
          <h2 className="text-xl font-bold text-blue-700 flex items-center">
            <FiCheckSquare className="w-5 h-5 mr-2 text-blue-600" /> Processing Orders
          </h2>
